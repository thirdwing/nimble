...
class NimArr<1, T> : public NimArrBase<T> {
  public:
    ...
    T &operator()(int i) {
        return((*NimArrBase<T>::vPtr)[calculateIndex(i)]);   // could add asserts here
    }
    ...
    NimArr<1, T> &operator=(const NimArr<1, double> &other) {
        return(templateCopyOperator<double>(other));
    }
    NimArr<1, T> &operator=(const NimArr<1, int> &other) {
        return(templateCopyOperator<int>(other));
    }
...
class NimArr<2, T> : public NimArrBase<T> {
  public:
    ...
    T &operator()(int i, int j) {
        return((*NimArrBase<T>::vPtr)[calculateIndex(i, j)]);   // could add asserts here
    }
...
class NimArr<3, T> : public NimArrBase<T> {
  public:
    ...
    int calculateIndex(int i, int j, int k) {
        return(NimArrBase<T>::offset + NimArrBase<T>::stride1 * i + stride2 * j + stride3 * k);
    } //k * s1s2 + j*s1 + i);}
    T &operator()(int i, int j, int k) {
        return((*NimArrBase<T>::vPtr)[calculateIndex(i, j, k)]);   // could add asserts here
    }
    ...
    virtual void setSize(vector<int> sizeVec) {
        setSize(sizeVec[0], sizeVec[1], sizeVec[2]);
    }
    virtual int numDims() const {
        return(3);
    }
...
class NimArr<4, T> : public NimArrBase<T> {
  public:
    ...
    int calculateIndex(int i, int j, int k, int l) {
        return(NimArrBase<T>::offset + NimArrBase<T>::stride1 * i + stride2 * j + stride3 * k + stride4 * l );
    } //k * s1s2 + j*s1 + i);}
    T &operator()(int i, int j, int k, int l) {
        return((*NimArrBase<T>::vPtr)[calculateIndex(i, j, k, l)]);   // could add asserts here
    }
    ...
    virtual void setSize(vector<int> sizeVec) {
        setSize(sizeVec[0], sizeVec[1], sizeVec[2], sizeVec[3]);
    }
    virtual int numDims() const {
        return(4);
    }
...
class VecNimArr : public VecNimArrBase<T>  {
  public:
    ...
    void resizeNoPtr(int i) {
        values.resize(i);
    }
    int getsizeNoPtr() {
        return(values.size());
    }
    virtual NimArrBase<T>* getBasePtr(int i) {
        return(&(values[i]));
    }
    virtual int size() {
        return(values.size());
