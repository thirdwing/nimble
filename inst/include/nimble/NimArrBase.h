...
class NimArrType {
  public:
    nimType myType;
    virtual nimType getNimType() const {
        return(myType);
    };
    virtual ~NimArrType() {};
};
...
class NimArrBase: public NimArrType {
  public:
    ...
    void setLength(int l) {
        NAlength = l;    // Warning, this does not make sense if vPtr is pointing to someone else's vMemory.
        v.resize(l);
    }
    void setMyType() {
        myType = UNDEFINED;
